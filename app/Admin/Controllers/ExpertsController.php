...
class ExpertsController extends Controller {
		...
		return Admin::grid( Expert::class, function ( Grid $grid ) {
			...
			$grid->actions( function ( $actions ) {
				if ( $this->row->trashed() ) {
					...
					// 添加一个查看按钮
					$actions->prepend( sprintf( '<a href="%s" target="_blank" title="%s"><i class="fa fa-eye"></i></a>', $this->row->expert_url, trans( 'admin.show' ) ) );
				}
			} );
	...
	protected function form() {
		return Admin::form( Expert::class, function ( Form $form ) {
			...
			$form->image( 'cover', '头像' )
			     ->move( 'images/experts/' . date( 'Y-m-d' ) )
			     ->uniqueName()
			     ->help( '请上传专家头像，图片尺寸为：482 x 682' );
